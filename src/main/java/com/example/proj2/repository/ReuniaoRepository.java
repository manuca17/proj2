...
public interface ReuniaoRepository extends JpaRepository<Reuniao, Integer> {
    ...
    @Query("SELECT r FROM Reuniao r LEFT JOIN FETCH r.idArtesa LEFT JOIN FETCH r.idProjeto WHERE r.idProjeto.id = ?1")
    List<Reuniao> findByIdProjetoIdWithRelations(Integer projetoId);

    @Query("SELECT r FROM Reuniao r WHERE r.idArtesa = ?1")
    List<Reuniao> findByIdArtesa(Artesa artesao);

    @Query("SELECT r FROM Reuniao r LEFT JOIN FETCH r.idArtesa LEFT JOIN FETCH r.idProjeto WHERE r.id = ?1")
